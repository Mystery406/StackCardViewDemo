...
public class StackCardTouchHelper extends ItemTouchHelper.Callback {
    ...
    public StackCardTouchHelper(RecyclerView recyclerView, MainActivity.CardAdapter adapter, List datas) {
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        /**
         * 由于我们不需要长按拖拽，所以直接传入s0即可，传入0代表不监听
         */
        int swipeFlags = ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT | ItemTouchHelper.UP | ItemTouchHelper.DOWN;
        //如果卡片不是位于最上方的就设置swipeFlag为0禁止个方向滑动
        return viewHolder.getLayoutPosition() == 0 ? makeMovementFlags(0, swipeFlags) : makeMovementFlags(0, 0);
    }
...
//            holder.iv_del.setAlpha(0f);
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
        ...
        Log.e(TAG, "actionState:" + actionState + ",isCurrentlyActive:" + isCurrentlyActive);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (level > 0) {
                //下面层,每一层的水平方向都要增大
                childView.setScaleX((float) (1 - StackCardConfig.SCALE_GAP * level + fraction * StackCardConfig.SCALE_GAP));
                if (level < StackCardConfig.MAX_SHOW_COUNT - 1) {
                    //1 2层
                    childView.setScaleY((float) (1 - StackCardConfig.SCALE_GAP * level + fraction * StackCardConfig.SCALE_GAP));
                    childView.setTranslationY((float) (StackCardConfig.TRANS_Y_GAP * level - fraction * StackCardConfig.TRANS_Y_GAP));
                    childView.setTranslationZ((float) (StackCardConfig.TRANS_Z_GAP * (StackCardConfig.MAX_SHOW_COUNT - 1 - level)
                            + fraction * StackCardConfig.TRANS_Z_GAP));
                } else {
